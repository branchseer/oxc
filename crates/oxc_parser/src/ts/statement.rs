...
impl<'a, A: oxc_span::ast_alloc::AstAllocator, H: crate::Handler<'a, A>> ParserImpl<'a, H, A> {
        ...
        match self.cur_kind() {
            ...
            Kind::NoSubstitutionTemplate | Kind::TemplateHead => {
                ...
                if node
                    .expressions
                    .specialize_ref()
                    .is_ok_and(|expressions| !expressions.is_empty())
                {
    ...
    pub(crate) fn parse_ts_type_signature(&mut self) -> Result<Option<TSSignature<'a, A>>> {
        if self.is_at_ts_index_signature_member() {
            return self.parse_ts_index_signature_member(None, None).map(Some);
        }
    ...
    pub(crate) fn parse_ts_type_assertion(&mut self) -> Result<Expression<'a, A>> {
        ...
        let type_annotation = self.ast.ts_type_assertion_annotation(self.end_span(span), ts_type);
        let lhs_span = self.start_span();
        let expression = self.parse_simple_unary_expression(lhs_span)?;
        Ok(self.ast.expression_ts_type_assertion(self.end_span(span), type_annotation, expression))
    }
